...
$(document).ready(function() {
    ...
    var checkAccount = setInterval(function(){
    	if ($('[class^="accountDetails"]').length > 0) {
				...
				var guildList = $('[class^="guild-"]').filter(function( index ) {
					...
					var match = classList.filter(value => /^selected-/.test(value));
					if (match.length > 0) {
						return $(this);
					}
			  });
    ...
    var checkRefresh = setInterval(function(){
		...
		function addTwitchSwitch() {
			...
			$('#gdq-header').after(`<div id="twitch-switch" style="position: fixed; width: ${$('div[class^="channels"]').width()}px; height: 22px; left: ${$('[class^="guildsWrapper"]').width()}px; bottom: 0; z-index: 1;"><label for="twitch-player-display" id="twitch-player-display-label">Twitch Player Embed</label></div>`);
